    ...
    struct Settings {
        ...
        static var stepperCell: UICollectionView.CellRegistration<UICollectionViewListCell, StepperSetting> {
            UICollectionView.CellRegistration { cell, indexPath, itemIdentifier in
                ...
                cell.accessories = [
                    UICellAccessory.customView(configuration: UICellAccessory.CustomViewConfiguration(customView: UIImageView(image: UIImage(systemName: "\(Int(abs(itemIdentifier.value))).circle.fill")?
                        .applyingSymbolConfiguration(UIImage.SymbolConfiguration(font: UIFont.preferredFont(forTextStyle: .title1)))?
                        .applyingSymbolConfiguration(UIImage.SymbolConfiguration(hierarchicalColor: itemIdentifier.value == 0 ? .tintColor : itemIdentifier.value < 0 ? .systemRed : .systemGreen))),
                                                                                                      placement: .trailing(),
                                                                                                      reservedLayoutWidth: .actual)),
