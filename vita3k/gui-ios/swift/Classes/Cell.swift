...
class Cell : UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        if #available(iOS 26.0, *) {
            ...
            visualEffectView.heightAnchor.constraint(equalTo: widthAnchor)
                .isActive = true
        } else {
    ...
    func set(game: Game, controller: UIViewController) {
        ...
        let imageURL: URL = artworkDirectoryURL.appending(component: "\(game.details.title.lowercased()).png")
        ...
        imageView.image = if fileExists {
            ...
            UIImage(contentsOfFile: game.details.iconPath)
        }

        textLabel.text = game.details.title
        secondaryTextLabel.text = ByteCountFormatter.string(fromByteCount: app_size(std.string(game.details.path),
                                                                                    std.string(game.details.titleIdentifier)),
                                                            countStyle: .file)
        ...
        optionsButton.menu = .init(children: [
            ...
            UIAction(title: "Delete", image: .init(systemName: "minus.circle"), attributes: .destructive) { _ in
                    let alertController: UIAlertController = .init(
                        title: "Delete \"\(game.details.title)\"?",
                        message: "",
                    ...
                    alertController.addAction(
                        .init(
                            ...
                            handler: { _ in
                                let task = Task {
                                    delete_app(std.string(game.details.path))
                                    guard let controller = controller as? GamesController else {
                                        return
                                    }
...
extension Cell: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey: Any] ) {
        ...
        let artworkDirectoryURL: URL = documentDirectoryURL.appending(component: "covers")
        let url: URL = artworkDirectoryURL.appending( component: "\(game.details.title.lowercased()).png")
        Task {
